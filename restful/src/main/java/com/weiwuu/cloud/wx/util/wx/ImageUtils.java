...
public class ImageUtils
{
	...
	public final static void pressImage(String pressImg, String srcImageFile,
		...
		try {
			...
			g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP,
					alpha));
			int widethR = ((wideth - wideth_biao) > 0 ? (wideth - wideth_biao)
					: 10) / 2;
			if ((wideth - wideth_biao) > 0) {
				widethR = (wideth - wideth_biao) / 2;
				g.drawImage(src_biao, widethR, (height - height_biao) / 2,
						wideth_biao, height_biao, null);
			} else {
				widethR = 10;
				g.drawImage(src_biao, widethR, (height - height_biao) / 2,
						wideth, height_biao, null);
			}
	...
	public static void main(String[] args) {
		...
		// // 方法二：按高度和宽度缩放
		// ImageUtil.scale2("e:/abc.jpg", "e:/abc_scale2.jpg", 500, 300,
		// true);//测试OK
		// // 2-切割图像：
		// // 方法一：按指定起点坐标和宽高切割
		...
		// // 5-给图片添加文字水印：
		// // 方法一：
		// ImageUtil.pressText("我是水印文字","e:/abc.jpg","e:/abc_pressText.jpg","宋体",Font.BOLD,Color.white,80,
		// 0, 0, 0.5f);//测试OK
		// // 方法二：
		...
		// try {
		// download("http://www.liantu.com/tiaoma/eantitle.php?title=ZGlURTBYRmd6YzRMV2ZwdUFEWDdhdz09",
		// "31.png", u+"/");
		// } catch (Exception e) {
		// }finally{
		// //放大图片
		// scale("d:/test/31.png", "d:/test/30.png", 2, true);
		// // 6-给图片添加图片水印：
		// pressImage("d:/test/30.png", "d:/test/2.jpg","d:/test/300.png", 0, 0,
		// 1);//测试OK
		// }
		...
		try {
			...
			ImageUtils
					.download(
							"http://www.liantu.com/tiaoma/eantitle.php?title=ZGlURTBYRmd6YzRMV2ZwdUFEWDdhdz09",
							picName, "d:/codepic/");
		} catch (Exception e) {
		} finally {
			// 放大图片
			String picUrl = "d:/codepic/" + picName;
			ImageUtils.scale(picUrl, picUrl, 2, true);
			// 6-给图片添加图片水印：
			ImageUtils.pressImage(picUrl, "d:/test/2.jpg", picUrl, 0, 0, 1);// 测试OK
		}
