$(function (undefined) {
    ...
    //会话列表
    namelist[groupid] = gname;//

    var conversationStr = '<li targetType="{0}" targetId="{1}" targetName="{2}"><span class="user_img" style="margin-right:0px;width:50px;"><img src={3} onerror="this.src=\'static/images/personPhoto.png\'"/><font class="conversation_msg_num {4}">{5}</font></span><span class="conversationInfo"><p style="margin-top: 10px"><font class="user_name">{6}</font><font class="date" >{7}</font></p></span></li>';
    ...
    $("div.listAddr li:lt(4)").click(function () {
        $("#notice").show().css({"color": "green"}).text("开始聊天").delay(2000).fadeOut("slow");
        getHistory(this.getAttribute("targetId"), this.getAttribute("targetName"), this.getAttribute("targetType"));
    });
    ...
    //初始化SDK
    RongIMClient.init(imappkey);

    $.ajax({
        type: "get",
        url: "http://proapi.weiwuu.net:8090/im/rc/gettoken?code=" + id,
        dataType: "json"
    }).done(function (data) {
        if (data.head.code == 1) {
    ...
    RongIMClient.getInstance().setOnReceiveMessageListener({
        onReceived: function (data) {
            ...
            if (currentConversationTargetId != data.getTargetId()) {
                if (document.title != "[新消息]") document.title = "[新消息]";
        ...
        $scope.ConversationList.forEach(function (item) {
            ...
            RongIMClient.getInstance().getUserInfo(item.getSenderUserId(), {
                onSuccess: function (info) {
                    console.log(info);
                    console.log(info.getUserId()+" | "+info.getUserName()+" | "+info.getPortraitUri());
                    _html += String.stringFormat(conversationStr, item.getConversationType().valueOf(), item.getTargetId(), namelist[item.getTargetId()], info.getPortraitUri(), item.getUnreadMessageCount() == 0 ? "hidden" : "", item.getUnreadMessageCount(), namelist[item.getTargetId()], new Date(+item.getLatestTime()).toString().split(" ")[4]);
    ...
    function getHistory(id, name, type, again) {
        ...
        if ($scope.historyMessages.length == 0 && !again) {
            ...
            RongIMClient.getInstance().getHistoryMessages(RongIMClient.ConversationType.setValue(conver), currentConversationTargetId, 10, {
        ...
        $scope.historyMessages.forEach(function (item) {
            ...
            RongIMClient.getInstance().getUserInfo(item.getSenderUserId(), {
                onSuccess: function (info) {
                    console.log(info);
                    console.log(info.getUserId()+" || "+info.getUserName()+" || "+info.getPortraitUri());
                    _html += String.stringFormat(historyStr, item.getMessageDirection() == 0 ? "other_user" : "self", item.getMessageDirection() == 1 ? owner.portrait :info.getPortraitUri(), "", item.getMessageDirection() == 0 ? 'white_arrow.png' : 'blue_arrow.png', myUtil.msgType(item), item.getMessageId());
        ...
        if (again == 1 && _html) {
            _html += "<div class='historySymbol'>以上为历史消息</div>";
        }
